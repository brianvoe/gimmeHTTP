import ShellHttpie from './shell.httpie'
import { Config, Http } from '../utils/generate'
import { describe, expect, test } from 'vitest'

describe('ShellHttpie.generate', () => {
  test('should build a basic GET request', () => {
    ...
    const config: Config = {}
    const result = ShellHttpie.generate(config, httpRequest)
    expect(result).toBe(`
...
`.trim())
  ...
  test('should build a GET request with URL parameters', () => {
    ...
    const result = ShellHttpie.generate(config, httpRequest)
    expect(result).toBe(`
http GET "https://example.com/?address.zip=66031&address.country=Wallis"
`.trim())
  })
  ...
  test('should build a GET request with array URL parameters', () => {
    ...
    const result = ShellHttpie.generate(config, httpRequest)
    expect(result).toBe(`
http GET "https://example.com/?tags=alpha&tags=beta&category=backend"
`.trim())
  })
