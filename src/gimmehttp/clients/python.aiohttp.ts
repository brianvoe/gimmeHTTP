...
export default {
  ...
  generate(config: Config, http: Http): string {
    ...
    builder.line('import aiohttp')
    builder.line('import asyncio')
    builder.line()
    builder.line('async def main():')
    builder.indent()
    ...
    builder.line()
    builder.line('async with aiohttp.ClientSession() as session:')
    builder.indent()
    builder.line(
      'async with session.%r(url%r) as response:',
      http.method.toLowerCase(),
      params.length > 0 ? `, ${params.join(', ')}` : ''
    )
    builder.indent()
    if (config.handleErrors) builder.line('response.raise_for_status()')
    builder.line('print(await response.text())')
    builder.outdent()
    builder.outdent()

    if (config.handleErrors) {
      builder.outdent()
      builder.line('except aiohttp.ClientError as e:')
      builder.indent()
