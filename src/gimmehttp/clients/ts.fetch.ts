...
export default {
  ...
  generate(config: Config, http: Http): string {
    ...
    if (wasInferred && parseMethod === 'json()') {
      builder.line('// Response Content-Type inferred as: %r', responseType)
    }

    if (config.handleErrors) {
      builder.line('.then((response: Response) => {')
      builder.indent()
      ...
      builder.line('.catch((error: unknown) => console.error("There was a problem with the fetch operation:", error));')
    } else {
      builder.line('.then((response: Response) => response.%r)', parseMethod)
      builder.line('.then((data) => console.log(data));')
    }
